...
- Multi-drug resistance (MDR) hub with colistin resistance: mcr carriage 44.4% (63/142 any mcr variant), sul1 (61.3%), floR (56.3%), mph(A)/mrx(A) (54.2%), blaTEM-1 (38.0%), mcr-1.1 (36.6%)
- **14.4 mean AMR genes** per plasmid
...
### High-AMR ESBL Lineage (pLIN 1.1.2.15.48.1434)
- **163 plasmids** predominantly IncFII (92.0%) with IncF (8.0%)
- **7.4 mean AMR genes** per plasmid
- Carries ESBL gene blaCTX-M-27 (29.4%), conferring 3rd-generation cephalosporin resistance
- Represents a major IncFII-dominated ESBL lineage relevant to empirical UTI treatment guidelines
...
## 4. 1.54-Fold Improvement in Discriminatory Power Over Inc Typing

- pLIN resolves **3,073 unique strain-level codes** from 8,077 plasmids (28 Inc/Rep groups)
- Simpson's Diversity Index: **D = 0.985** (pLIN) vs **D = 0.641** (Inc typing alone)
- **97.8%** of L6 codes (3,006/3,073) contain plasmids from a single Inc group, confirming pLIN captures known taxonomy as an emergent property
...
- Scaled from 8,077 training to **79,305 plasmids** (+ 71,249 NCBI RefSeq references)
- Resolves **57,886 unique pLIN codes** -- an 18.8-fold increase from 3,073 training codes
- KNN classifier achieves **97.3% classification rate** (91.1% accuracy, k=5, cosine distance)
...
- IncFII plasmids carry **both** the highest virulence factor rate (23.2%) and substantial AMR (62.3%)
...
|----------------|------------------------|
| pLIN 1.1.2.15.48.671: 100% KPC-positive (n=90), 13.2 AMR genes, immediate isolation trigger | "IncN" (shared by >1,000 plasmids with vastly different profiles) |
| Cross-Inc MDR hub with colistin resistance (44.4% mcr carriage) exchanging multi-drug resistance cassettes | Individual replicon types with no cross-Inc linkage |
