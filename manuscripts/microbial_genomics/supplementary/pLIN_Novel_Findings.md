...
- 44.4% mcr colistin resistance carriage (63/142 any mcr variant), **14.4 mean AMR genes** per plasmid
...
## 3. Comprehensive AMR Landscape Across 6,998 Gram-Negative Clinical Plasmids (20 Inc Groups)
...
- pLIN resolves **3,073 unique strain-level codes** from 8,077 training samples (8,056 unique) across 28 groups
- Simpson's Diversity Index: **D = 0.985** (pLIN) vs **D = 0.641** (Inc/rep typing alone)
- **97.8%** of L6 codes (3,006/3,073) contain plasmids from a single group, confirming pLIN captures known taxonomy as an emergent property
...
- Scaled from 8,056 unique training to **79,305 plasmids** (8,056 unique training + 71,249 reference)
- Resolves **57,886 unique pLIN codes**
- KNN classifier achieves **97.3% classification rate** (91.1% accuracy across 28 groups, k=5, cosine distance) with 2,109 low-confidence sequences
...
|----------------|------------------------|
| pLIN 1.1.2.15.48.671: 100% KPC-positive, 13.2 AMR genes, immediate isolation trigger | "IncN" (shared by >1,000 plasmids with vastly different profiles) |
